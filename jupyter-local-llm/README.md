...
| 用途 | 仕組み |
|------|--------|
| ① コード補完 | `%%complete` マジック / `completion_panel()`（**自前実装・jupyter-ai 不要**）|
...
> JupyterLab セル上に直接出る“ゴーストテキスト補完”はフロントエンド拡張が必要なため、
> 純 Python では上記（マジック / パネル）で代替している。
...
│   ├── complete.py    # コード補完（code_complete / %%complete / completion_panel）
