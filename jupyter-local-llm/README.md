...
| `notebooks/08_copilot_research.ipynb` | Copilot Research（M365 Copilot × 擬似GEPA・目次→章別リサーチ→統合）の使い方 |
...
- **検索の構造**: 範囲確定（collection=OR/タグ=AND/doc_id）→ 文書候補選定（top-N）→
  各文書内で **ベクトル候補と独立字句候補を union** →
  正規化スコアで再ランク → 文書ごとの上限 → 全体のコンテキスト予算。
  字句候補は英数トークン（規程番号・金額・型番）の完全一致 + 日本語文字2-gram で、
  ベクトル上位候補の外にある完全一致も救済する（設定:
  `vector_candidate_k_per_doc`/`lexical_candidate_k_per_doc`/`context_budget`）。
  `use_graph=True` は graph 索引がある文書のみ BookRAG 検索、無ければ通常RAGへ
  フォールバック（生成回答は `graph_answer` として根拠と分離）。
- **文書の所在（membership）と内容を分離**: 同一内容が複数フォルダにあっても
  doc_id は1つで、所在は memberships.json に記録される。一方のフォルダの
  ファイルだけを改訂しても、他フォルダに残る旧文書の索引は消えない。
- **取り込みはバッチ処理**: 埋め込み・ベクトル挿入は `ingest_batch_size`（既定32）
  件ずつ。数十MB文書でも全チャンクを一括処理しない。途中失敗は再実行時に
  掃除され二重登録しない。
- **回答生成は階層 Map-Reduce**: 文書ごとの部分回答（answered/no_info/failed を
  記録）→ 6件グループで中間統合 → 最終統合。50/100文書でも巨大プロンプトを
  作らず、どの文書も黙って落ちない。
- **ナレッジグラフの可視化**: 文書詳細の「グラフ表示」タブ（外部CDN不使用の
  SVG。検索/種別フィルタ/1〜2ホップ/上限50-200/ノード選択で根拠表示）と
  `GET /api/docs/graph-data`。安全モードで一部ノードのみ抽出した場合は
  **部分グラフ**（カバレッジ%）と明示される。
...
| バージョン | 内容 |
|-----------|------|
| **0.9.1** | 再監査修正 — 文書改訂のトランザクション化（新版失敗でも旧版・所在・検索が生存）・キャッシュヒット時の所在移動・タグ/membershipの整合（残存行から再計算・set_tags同期）・コレクション文書数をユニークdoc_idで集計・rebuildの所属維持・取り込みの真のストリーム化（read_text全量読込の廃止・JSONL→最終JSONストリーム生成・force失敗後の索引復旧）・Entity埋め込み128件バッチ・英数トークンの完全一致（部分文字列一致の廃止）・200文書打ち切り撤廃・グラフGUIのwindowリスナーリーク修正（pointer capture）・E2E動的ポート/skip・ruff実測クリーン（E501含む） |
| 0.9.0 | 監査修正＋ナレッジグラフ可視化 — 共有文書の改訂で他フォルダの索引を消さない（所在=membership 分離）・単一ファイルのタグ保存・スコープ AND/OR 統一（collection=OR/タグ=AND）・取り込みのバッチ化（ingest_batch_size・中途失敗の掃除・巨大テキストのセグメント読込）・グラフのカバレッジ記録（部分グラフ表示）・Map-Reduce の階層化（予算管理・50/100文書対応・文書別状態の追跡）・真のハイブリッド検索（独立字句候補との union・日本語2-gram・完全一致の救済）・ETA の phase_id 分離（フォルダ/Map 処理でも機能）・**ナレッジグラフ可視化**（graph-data API + 文書詳細のタブ + 外部依存なし SVG グラフ + 根拠の追跡ID） |
