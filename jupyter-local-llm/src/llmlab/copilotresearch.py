...
class Candidate:
    ...
    def __init__(self, instruction: str, parent: str | None, born_iter: int, rationale: str = ""):
        Candidate._seq += 1
        self.id = f"c{Candidate._seq}"
...
def _orchestrate(run: _Run) -> None:  # noqa: C901  本流は一本で読める方が保守しやすい
    ...
    try:
        ...
        emit({"type": "outline", "title": outline["title"], "chapters": outline["chapters"]})
        chapters = outline["chapters"]
        ...
        keys = [c["title"] for c in minibatch]
        ...
        def evaluate(cand: Candidate, it: int) -> None:
            for i, ch in enumerate(minibatch):
                ...
                s, det = judge(ch, res)
                cand.scores[ch["title"]] = s
                cand.details[ch["title"]] = det
                cand.results[ch["title"]] = res.to_dict()
            ...
            emit({"type": "gepa_candidate", "iter": it, **cand.to_dict(front_ids)})

        stage("gepa")
        seed = Candidate(_SEED_INSTRUCTION, parent=None, born_iter=0, rationale="初期指示（シード）")
        pool.append(seed)
        evaluate(seed, 0)
        ...
        for it in range(1, budget + 1):
            ...
            child = Candidate(instr, parent=parent.id, born_iter=it, rationale=why)
            pool.append(child)
            evaluate(child, it)
