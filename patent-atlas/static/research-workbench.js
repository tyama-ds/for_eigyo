...
function captureResearchPlan(){
 ...
 researchPlanDraft.concepts=$$('.research-concept').map(el=>({id:el.dataset.concept,selected:$('input[data-use]',el).checked,role:$('select',el).value,terms:$('textarea',el).value.split(/\r?\n/).map(t=>t.trim()).filter(Boolean)}));
...
function researchPlanState(plan){
 if(!plan)return null;
 if(researchPlanDraft?.id!==plan.id)researchPlanDraft={id:plan.id,concepts:plan.concepts.map(c=>({...c,selected:c.role==='required',role:c.role==='exclude'?'exclude':'required',terms:[...c.terms]})),classification_keys:[],preview:null};
...
function researchPlanHtml(plan){
 ...
 return `<div class="panel-heading"><div><span class="eyebrow">CONCEPTS → QUERY</span><h2>観点を組み立てる</h2></div><span class="pill">${plan.method==='llm'?'LLMによる案':'入力の整理'}</span></div><p class="muted">${esc(plan.summary)}</p><p class="caption">採用する観点にチェック。同じ観点の語はOR、観点同士はANDで結びます。除外は明示的に選んだ観点だけです。</p>
 <div id="research-concepts">${plan.concepts.map(c=>{const d=draft.concepts.find(x=>x.id===c.id);return `<article class="research-concept" data-concept="${esc(c.id)}"><div class="panel-heading"><label class="check"><input data-use type="checkbox" ${d.selected?'checked':''}><b>${esc(c.name)}</b></label><select aria-label="${esc(c.name)} の条件"><option value="required" ${d.role==='required'?'selected':''}>含める · AND</option><option value="exclude" ${d.role==='exclude'?'selected':''}>除外 · NOT</option></select></div><label>語句 · 1行に1語句<textarea rows="3" maxlength="5000">${esc(d.terms.join('\n'))}</textarea></label><p class="caption">${esc(c.reason)}${c.evidence_ids?.length?' / 根拠：'+c.evidence_ids.map(esc).join('、'):''}</p>${c.abstract_terms?.length?`<div class="research-expansion"><span>機能・概念を広げる</span>${c.abstract_terms.map(t=>`<button class="button small" data-abstract="${esc(t)}" type="button">＋ ${esc(t)}</button>`).join('')}</div>`:''}</article>`;}).join('')}</div>
...
function researchPlanPayload(){captureResearchDraft();if(researchBriefChanged())throw new Error('入力条件が観点案の作成後に変わっています。観点をもう一度起案してください。');captureResearchPlan();return {revision:state.research_workbench.revision,plan_id:researchPlanDraft.id,concepts:researchPlanDraft.concepts.filter(c=>c.selected).map(({id,role,terms})=>({id,role,terms})),classification_keys:researchPlanDraft.classification_keys,acknowledge_unmapped:researchPlanDraft.acknowledge_unmapped};}
