...
function captureResearchDraft(){
 if(typeof captureInventionDraft==='function')captureInventionDraft();
...
function invalidateResearchPreview(){if(researchPlanDraft)researchPlanDraft.preview=null;if($('#research-query-preview'))$('#research-query-preview').innerHTML='';const notice=$('#research-stale-plan');if(notice)notice.hidden=!(state.research_workbench?.plan&&researchBriefChanged());if(typeof invalidateInventionPreview==='function')invalidateInventionPreview();}
...
function renderResearchWorkbench(){
 ...
 if(mode!=='examples'&&mode!=='tools'&&typeof inventionWorkbenchHtml==='function'){
  const legacy=$('.research-plan',host),layout=$('.research-layout',host);
  if(legacy){const details=document.createElement('details');details.className='research-legacy-plan';details.innerHTML='<summary>キーワード・分類から直接組み立てる（従来の観点案）</summary>';host.append(details);details.append(legacy);}
  if(layout){layout.classList.add('research-invention-brief');layout.insertAdjacentHTML('afterend',inventionWorkbenchHtml());}
  const actions=$('.research-plan-actions',host);if(actions){const details=document.createElement('details');details.className='research-direct-actions';details.innerHTML='<summary>キーワードから直接、観点案を作る</summary>';actions.replaceWith(details);details.append(actions);}
 }
 ...
 bindResearchApply();refreshResearchLogic();if(typeof bindInventionWorkbench==='function')bindInventionWorkbench();
}
