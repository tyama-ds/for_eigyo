...
function queryCard(q) {
 ...
 const terms=q.term_catalog||[...new Set([...(q.keywords||[]),...(q.include_terms||[]),...(q.exclude_terms||[])])], edits=outputEdits.get(q.id)||{};
 ...
 ${typeof classificationChangesHtml==='function'?classificationChangesHtml(q.classification_changes):''}<details class="source-expression"><summary>元の設計式・変更内容</summary><pre class="expression">${esc(q.expression)}</pre><p class="caption">${q.boolean_tree?'観点内OR・観点間AND、または取り込み式の論理構造を保持しています。':'キーワードはAND、分類はOR。追加語はOR群をANDで結び、除外語はNOTにします。'}</p>${q.removed_terms?.length?`<p class="caption">前回から解除した語：${q.removed_terms.map(esc).join('、')}</p>`:''}${q.known_keep_coverage?`<p class="caption">追加語の条件で残る既知の必要特許：${q.known_keep_coverage.retained} / ${q.known_keep_coverage.total} 件（タイトル・要約の文字列照合）。</p>`:''}</details></div>`;
}
