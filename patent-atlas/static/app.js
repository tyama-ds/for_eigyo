...
let state, step = 0, activeTab = 'explore', tool = 'lasso', picked = new Set(), pendingFile, toastTimer, animation, selectionSave = Promise.resolve(), shownJob;
...
function setTab(tab) {
 ...
 if(tab==='explore')renderStep(); if(tab==='agent')renderAgent(); if(tab==='discovery')renderDiscovery();if(tab==='orchestration')renderOrchestration();
...
function fillSettings() {
 const s=state.settings; $('#provider').value=s.provider; $('#base-url').value=s.base_url; $('#llm-model').value=s.model;
...
async function uploadFile(file,mapping={}) {
 if(!file)return;if(file.size>20*1024*1024)throw new Error('CSVは20MB以下にしてください。');pendingFile=file;pendingImportMode=$('#csv-import-mode')?.value||pendingImportMode;toast('CSVを読み込み、特許を配置しています…');
 ...
 if(data.needs_mapping){showMapping(data);return;}state=data;picked.clear();$('#mapping-dialog').close();setStep(2);toast(`${state.patents.length} 件を読み込みました。`);
}
function showMapping(data) {const fields={title:'発明の名称 *',abstract:'要約',id:'公報番号',applicant:'出願人',ipc:'IPC',fi:'FI',fterm:'F-term',year:'公開日',family:'ファミリーID'};$('#mapping-fields').innerHTML=Object.entries(fields).map(([k,v])=>`<label>${v}<select data-field="${k}" ${k==='title'?'required':''}><option value="">割り当てなし</option>${data.headers.map(h=>`<option value="${esc(h)}" ${data.mapping[k]===h?'selected':''}>${esc(h)}</option>`).join('')}</select></label>`).join('');$('#mapping-dialog').showModal();}
...
function showPatent(row) {
 ...
 $('#patent-detail').innerHTML=`<div class="detail-id">${esc(row.id)} ${state.demo?'· 架空':''}</div><h3 class="detail-title">${esc(row.title)}</h3><p class="detail-text">${esc(row.abstract||'要約がありません。')}</p><div class="detail-block"><b>出願人・分類</b>${esc(row.applicant||'不明')}<br>IPC：${esc(row.ipc||'未取込')}${row.fi?`<br>FI：${esc(row.fi)}`:''}${row.fterm?`<br>Fターム：${esc(row.fterm)}`:''}</div><div class="detail-block"><b>なぜ、このあたり？</b>${esc(row.map_reason)}<br>${esc(row.inclusion_reason)}<br>群：${esc(state.clusters.find(c=>c.id===row.cluster)?.name||'')}</div>${row.label_reason?`<div class="detail-block"><b>${row.label_source==='agent'?'エージェント':'あなた'}の判断</b>${esc(row.label_reason)}</div>`:''}${row.score!=null?`<div class="detail-block"><b>${row.score_source==='llm'?'LLMの関連度（自己評価）':'モデルの関連度'} ${Math.round(row.score*100)} / 100</b>検索関連性の保証された確率ではありません。</div>`:''}`;
}
...
async function init(){
 [state,formatCatalog]=await Promise.all([api('/state'),api('/query/formats')]);fillSettings();updateChrome();renderStep();
 ...
 on('#cancel-mapping','click',()=>$('#mapping-dialog').close());on('#mapping-form','submit',async e=>{e.preventDefault();const mapping=Object.fromEntries($$('#mapping-fields select').map(s=>[s.dataset.field,s.value]));await uploadFile(pendingFile,mapping);});
