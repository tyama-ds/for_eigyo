...
- **情報源の管理** — UI から フィードの 追加 / 有効化・無効化 / 削除。各行に取得状態
  ...
  失敗理由（プロキシ／同意ページ／403／TLS証明書／解析）を切り分けられる。
- **取得の堅牢化** — ブラウザ相当の User-Agent と Google 同意回避クッキーを送信。
  Google ニュース RSS が社内プロキシ等で取得できない場合は **Bing ニュース RSS へ自動
  フォールバック**（逆も可）。フィードでない応答（同意/ブロックページ）は「非フィード応答」
  として明示。社内プロキシが HTTPS を傍受する環境向けに **CA証明書（ca_bundle）**を
  設定で指定可能（TLS 検証は常に有効）。
- **オフラインでも空にならない** — どのフィードにも接続できないときは、内蔵の
  サンプル記事（オフラインデモ）で画面を満たし、状態バーに「オフラインデモ」を表示。
- **キーボード操作** — `/` 検索・`F` 横断検索・`R` 再取得・`T` テーマ・`L` 表示・
  `B` 保存済み・`S` 情報源・`A` AIアシスタント・`1`〜`9` カテゴリ・`Esc` 閉じる（`?` で一覧）。
