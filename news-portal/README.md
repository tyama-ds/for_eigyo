...
- **ソース別表示** — 記事カード／ヒーロー／検索結果の**情報源名をクリック**すると、その
  ...
  のチップが出て、`×`／カテゴリ選択で解除。
- **過去ログ（自動アーカイブ）** — 取得した記事は表示上限とは別に `archive.jsonl` へ
  **自動で全件保存**される（1行1記事のJSON・idで重複排除・最大2万件で古い順に間引き・
  デモ記事は対象外）。フィードが入れ替わっても過去の記事を検索できる。
- **リサーチ（過去ログ×AI）** — 上部のリサーチボタンで専用ウィンドウを起動。
  **現在表示中＋過去ログ全体をキーワード横断検索**（スペース区切りAND）し、ヒットした
  記事をチェックボックスで選択（既定は上位20件・最大30件をAIへ）。右ペインで
  ローカルLLM等に「時系列で要約」「比較して」等の作業を依頼できる。推論モデルの
  思考過程は折りたたみ表示。
...
| GET | `/api/archive/search?q=&limit=` | 現在＋過去ログの横断検索（空白区切りAND・新着順・q空なら新着一覧） |
| GET | `/api/archive/stats` | 過去ログの件数と期間 |
