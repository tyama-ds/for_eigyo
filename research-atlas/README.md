...
重心・語句・変化の判定は、地図に表示する最大400論文の範囲です。別の画面の全件集計と区別してください。月別・3か月別には出版月が確認できる論文だけを使い、年のみの記録を1月へ割り当てません。日付不足の除外件数、空の期間、比較期間間の欠落を表示します。空の層は取得した表示標本内の0件であり、分野全体の0件ではありません。

内容変化の判定には、再投影用に保存した文書表現（最大50次元。必要時にSVDで縮約）を使います。ゼロベクトルを除く有効な論文が各期5件以上ある場合に199回のラベル置換検定を行い、同じ期間区分の全比較にBenjamini–Hochberg補正を適用します。q値0.05以下かつ重心のコサイン距離0.03以上を探索上の変化候補とします。この閾値は科学的な普遍基準ではありません。「検出せず」は変化がないという証明ではなく、少数標本・期間の欠落・未分類群は判定を保留します。投影法を変えても、同じ文書表現に対するこの判定は変わりません。
...
新しい分析結果は再投影用の入力を保存します。旧NMF結果では保存済みトピック重みを使用し、それ以外の旧結果で入力が残っていない場合は表示文献だけからTF-IDFを再構築した旨を表示します。元のSBERT等の表現で比較したい場合は再分析してください。UMAPの配置パラメーターについては[公式解説](https://umap-learn.readthedocs.io/en/latest/parameters.html)を参照できます。
...
| 技術マップ | 最大400論文を抽出し、自動／t-SNE／PCA／UMAPで2次元へ配置。共通座標の年・四半期・月別層、重心・内容変化・根拠付き解釈を表示。点は論文、類似線は文書表現に基づき、赤矢印は期間別重心の差。引用関係の線ではありません |
