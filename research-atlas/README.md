...
20,000件ずつの追加と多年分析は、[大規模データの検証記録](docs/large-corpus-validation.md) に実測を記載しています。v2.0では蓄積件数の固定上限を撤去し、重心分析を簡易版／全件対応版で選べます。[全件重心と代表論文レポートの検証記録](docs/centroid-validation-2.0.md) に25万件の重心照合とSQLite保存からCSVまでの結果を記載しています。実際の処理可能量は手法とPCのメモリ・ディスクに依存します。
...
論文CSVは**ファイル容量の固定上限なし・1ファイル20,000行まで**受け付けます。**データセットの蓄積件数には固定上限を設けません。** 分析期間は開始年・終了年を含めて最大50年です。CSV内の改行入り抄録は1レコードとして扱います。
...
| 蓄積と語彙ベースの分析 | 件数の固定上限なし、最大50年。選択手法の作業メモリ・近傍数等の条件による |
| 分野・キーワード・引用・年次件数 | 選択期間の全対象論文から集計 |
| 技術マップ・等高線 | 点と密度は最大400論文の表示標本。重心・件数・代表論文は簡易／全件を選択 |
...
NMFの大規模処理、LDAのオンライン学習、明示的に選ぶMiniBatch K-meansでは分割学習を使います。各論文のテーマ割り当て、年次集計、引用集計は表示用の400論文に限定しません。処理時間・必要メモリ・保存容量は抄録の長さ、語彙、著者・所属・参照の数、選択したモデル、PCによって変わります。固定件数の撤去は無制限のメモリや完了時間を保証しません。大規模データではまずNMFやMiniBatch K-meansの処理量を確認してください。

### 引用の流れ：新しい研究と、その参照基盤（v2.1）

左の **「引用の流れ」** は技術ランドスケープとは別の図です。共通のPCA座標で論文を配置し、出版年・四半期・月の層へ展開します。光と矢印は **引用する新しい論文 → 参照された以前の論文** を示します。線は取得済みの参考文献をDOI・標準識別子で照合した関係だけで、内容の近さや累積被引用数から作りません。時層の展開・再生、一時停止、層の間隔調整、ドラッグ回転に対応し、OSの「動きを減らす」設定では動きを抑えます。

期間・分野ごとに、新しい論文の内容重心と、その論文群の参照基盤の内容重心を計算します。新しい論文の重心は有効な文書ベクトルの等重み平均です。参照基盤は、各引用元論文が参照する、照合済みで以前に発表された文献のベクトルをまず平均し、次に引用元論文間で平均します。参考文献の多いレビュー論文だけが大きな重みを持たない方式です。両者の内容差には、引用元・参照先の両方に有効な表現がある同じ論文集合の「比較用の新論文重心」を用い、全新論文の重心と区別します。

分析は保存された分析対象全体で集計します。図の最大240論文・480引用線への間引きは集計値を変えません。CSV・JSON出力はその時の集計・重心・取得範囲と描画用の点・線を含み、全引用リンク一覧の出力ではありません。本文やLLM接続情報は外部へ送信しません。

分析対象外の参考文献は外部から自動取得せず、未照合として件数を表示します。曖昧な識別子、自己参照、未来の文献、出版時期の前後を確定できない関係は描画対象から除外します。年しか分からない同年論文の順序や、出版月を推測しません。参考文献の未取得と、取得された空の参考文献リストを区別します。引用は参照の記録であり、因果的な影響・結果への賛同・研究の優劣を示しません。出版期間に沿った回顧的な比較で、当時のデータベース収録状況や将来予測を再現するものではありません。

`GET /api/results/{id}/citation-flow?interval=year&topic_id=all` と、その `/export?format=csv|json` で同じ分析を利用できます。
...
### 投影を切り替えて、話題の重心を時層で追う

技術マップの「座標の投影」で **自動選択／t-SNE／PCA／UMAP** を切り替えられます。初期値は自動選択で、通常はt-SNE、少数・重複が多い標本ではPCA/SVDの線形投影を使います。分析設定でも初期投影を指定できます。マップ上の切替は表示論文だけを再投影し、テーマの分類・全件の集計・予測値を変更しません。UMAPは通常の依存ライブラリに含まれます。既存環境では `python -m pip install -r requirements.txt` で更新してください。

1. 「時間の単位」で **年別／3か月別／月別** を選びます。3か月別は暦年の四半期（1〜3月、4〜6月、7〜9月、10〜12月）です。
2. **平面／立体／時層に展開** を切り替えます。時層は共通のXY座標のまま期間ごとに開き、点と重心がアニメーションします。最大8層を同時表示し、前後の期間へ移動できます。OSの「動きを減らす」設定ではアニメーションと点滅を抑えます。
3. 「重心を追う話題」を選びます。点滅は期間・話題別の等重み重心です。**赤矢印は前後の重心を同じ時間平面へ投影した差**で、層を離す高さを移動距離に加えません。線の有無・破線と、内容変化の判定を確認してください。
4. マップ下の比較で、平面上の移動距離、投影前の表現のコサイン距離、前後の特徴語と根拠論文を確認できます。「解釈を生成」で計算結果のレポート、任意のローカルLLM／OpenAIの評論を保存できます。レポートのCSVには計測値・解釈・警告・根拠を含めます。

**全期間の表示標本を一度だけ投影して、その座標を年・四半期・月の各層で共有します。期間ごとに別の地図を学習しません。** 投影方法を変えると地図の座標系自体は変わります。期間の表示を切り替えても、同じ投影方法内の座標は変わりません。

「重心分析の範囲」で**簡易版／全件対応版**を選びます。簡易版は表示用の最大400論文で重心・特徴語・代表論文を計算します。全件対応版は保存された分析結果の全対象論文を使い、描画する点だけを間引きます。全件とは取り込み済みの分析対象であり、外部サービスの未取得論文や分析年範囲外の論文は含みません。月別・3か月別は出版月が確認できる論文だけを使い、年のみの記録を1月へ割り当てません。

全件版の自動投影はPCAです。PCAは全件で学習し、全点の配置から重心を計算します。UMAPは最大2,000件の参照点で学習し、同じモデルで全論文を配置する方式です。参照学習を使う場合は近似である旨を表示します。全件t-SNEは4,000件までで、これを超える場合は全件PCA／UMAPまたは簡易版を選びます。月・年の切替では同じ座標を再利用します。

分野タイトル・凡例をクリックすると、選択クラスター、右側の詳細、重心分析が切り替わります。重心の点または「代表論文を見る期間」を選び、**「重心付近の代表論文を分析」**を押すと、投影前の内容重心に近い最大6論文を選んでレポートを生成します。計算結果モードは原文抜粋と読解観点を表示し、ローカルLLM／OpenAIでは各論文の対象・手法・報告された結果・確認すべき点と共通点を評論します。近傍であることは品質・重要性・将来性の順位ではありません。数値照合に問題がある評論も警告付きで保持し、CSVへ出力できます。

立体・時層表示はドラッグで視点を回転できます。「補助グリッド」は初期OFFで、ONにすると各軸を4分割する薄い線を表示します。立体では密度の表面、時層では各平面に沿い、回転にも追従します。平面表示では非表示です。回転はブラウザ内の表示だけを変更し、Pythonで再分析しません。点・重心のクリックと回転を区別し、ホイールによるページスクロールを維持します。平面・立体・時層の切替では展開アニメーションが動きます。

内容変化には保存した文書表現（最大50次元。必要時にSVDで縮約）を使います。ゼロベクトルを除く有効な論文が各期5件以上ある場合に199回のラベル置換検定を行い、同じ期間区分の全比較にBenjamini–Hochberg補正を適用します。全件版の重心・cosine距離は全有効論文から求め、検定だけは各群最大200件の乱数固定標本を使います。大きな群のp値・q値は近似であり、検定対象数を別記します。q値0.05以下かつ内容重心のcosine距離0.03以上を探索上の変化候補とします。普遍的な科学的基準ではなく、少数標本・期間の欠落・未分類群は判定を保留します。

重心の移動は、期間ごとに集まる論文の平均的な内容構成が変わった可能性を示します。研究者の移籍、技術の物理的移動、因果的影響、実用化や将来の流行を直接示しません。LLMには計測値と前後各期最大6論文の抄録抜粋を渡し、語の変化・対象・手法・代替説明を評論させます。数値や根拠の不足は警告付きで表示し、LLMの通信・形式エラー時も計測値と定型解釈を保持します。

新しい分析結果は全論文で学習した共通基底の文書ベクトルを論文ごとに保存します。旧NMF結果では保存済みトピック重みを使用し、未割当成分が復元できない場合は明示します。それ以外の旧結果では選択範囲（簡易／全件）からTF-IDFを再構築し、元のSBERT等の空間ではないと表示します。元の表現で比較したい場合は再分析してください。UMAPの配置パラメーターは[公式解説](https://umap-learn.readthedocs.io/en/latest/parameters.html)を参照できます。
...
この反復探索・内容評価は、初期分析の対象10,000論文までです。大規模な全体分析では、まず分野・年次動向・技術マップの全件対応版で対象を見つけ、期間を絞った別の分析で利用してください。この反復探索の上限は、全件の重心分析や重心近傍論文の評論とは別です。
...
ローカルLLMの最終回答は、本文先頭の完結した `<think>…</think>` と、最終JSON全体を囲むコードフェンスに対応しています。別フィールドの `reasoning_content` / `thinking` は評論として扱いません。思考内からJSONを拾うことはせず、未完了の思考・最終回答の欠落・トークン上限・通信切断は生成失敗になります。ランドスケープの評論では、生成失敗と数値照合の警告を区別して理由を表示します。生成失敗時の計算結果は「計算結果の説明（代替表示）」として折りたたみ、数値照合に警告のある生成済み評論は本文を表示します。過去の失敗からLLM本文を得るには再生成が必要です。
...
CSVのファイル容量と結合後の蓄積件数には固定上限を設けません。アップロードは一時ファイルに保存し、文字コードの検証・CSVの読取を順に行います。実際に扱える容量はPCの空きディスク・メモリに依存します。論文CSVは1ファイル20,000行までです。無効な行や重複を報告し、空欄の引用数は未知のまま保持します。検索結果の一部のみを出力すると、その部分集合の分析になります。
...
LLMはResearch Atlasと別のPCでも動かせます。接続先にはPythonが動くPCから到達できるHTTP(S)のIP・ホスト名を指定します。`localhost` / `127.0.0.1` / `::1` はResearch AtlasのPythonが動くPC自身を指します。

- **別PCのLM Studio**：「[Serve on Local Network](https://lmstudio.ai/docs/developer/core/server/serve-on-network)」を有効にしてAPIサーバーを起動し、例えば `http://192.168.1.50:1234/v1` を指定します。IP・ポートはLM Studioに表示された接続先へ置き換えてください。
- **別PCのOllama**：[公式FAQ](https://docs.ollama.com/faq#how-can-i-expose-ollama-on-my-network)に従ってLLM側でネットワークから接続できるよう設定し、例えば `http://192.168.1.50:11434` を指定します。
- **ホスト名・IPv6**：`https://llm.example.internal/v1` や `http://[fd00::50]:1234/v1` も指定できます。接続先で認証が必要なら「ローカルAPIキー」に入力します。URLへ認証情報を埋め込む形式や、クエリ・フラグメントは受け付けません。

「LLM接続を確認」でモデル一覧を取得してから「このブラウザに保存」を押します。別PCの場合はLLM側の待受設定とファイアウォールでResearch AtlasのPCからの接続を許可してください。LLMとの通信は同じPC・別PCともに直接接続です。URL・モデル・APIキーは引き続きブラウザだけに保存します。

Ollamaでクラウド転送と判別できるモデルは使用しません。互換API側も、利用者が自前のモデルを推論するサーバーとして構成してください。モデルが未起動・未導入なら数値ベースのレポートを利用できます。生成失敗時も数値レポートとCSVは保存されます。
...
flowchart LR
    ...
    C --> D[K-means系 / 密度・近傍 / GMM / 階層 / NMF / LDA / BERTopic]
    D --> M[技術マップ・月次の変化]
    B --> E[著者・キーワード・引用の時系列]
    D --> F[論文件数の予測と過去検証]
    E --> G
...
| --- | --- | --- |
| K-means | ランダム初期化で指定数に分類 | 全件で必要時SVD≤50次元＋L2正規化した文書表現 |
| K-means++ | 同じK-meansの初期中心をk-means++で選ぶ | 同上 |
| MiniBatch K-means | 全論文を分割して3周学習、全件分類 | 同上 |
| X-means | 初期群をBIC基準で分割、最大群数まで増やす | 同上 |
| k-NNグラフ | 相互k近傍の連結成分。教師ありk-NN分類器とは異なる | 同上 |
| DBSCAN | epsと最小近傍数による密度分類、ノイズを保持 | 同上 |
| GMM | 対角共分散のガウス混合、最大所属確率へ割当 | 同上 |
| BIRCH | 全件をCF木へ集約し、部分群をWard法で統合 | 同上 |
| 階層クラスタリング | Ward法で全点を順に統合 | 同上 |
...
NMF・LDA・語彙ベースの分類に固定の総論文件数上限はありません。SBERT／旧Transformerの文書表現生成とBERTopicは別の作業条件として最大20,000論文です。Ward法は6,000件、BIRCHはCF部分群4,000、近傍グラフは有向関係800万、分類表現は512MiB、GMMの所属確率作業領域は256MiBを上限とし、超える場合は条件変更を案内します。レポートが扱う分類済み群は最大100個です。別手法への変更や論文の間引きを黙って行いません。各推定器・処理方式・実際の分類数はモデル情報に保存します。

「クラスタリングの詳細設定」ではDBSCANのeps・最小近傍数、k-NNの近傍数、X-meansの最大群数、BIRCHのしきい値を指定できます。新しい9方式は全件で共通の最大50次元表現を作ってから分類します。旧K-meansの自動MiniBatch切替は廃止し、MiniBatchを明示的に選択します。X-meansは原論文のBICによる分割方式を実装したもので、木構造による高速化は含みません。

参考：[scikit-learnのクラスタリング](https://scikit-learn.org/stable/modules/clustering.html)、[GMM](https://scikit-learn.org/stable/modules/generated/sklearn.mixture.GaussianMixture.html)、[X-means原論文](https://www.cs.cmu.edu/~dpelleg/download/xmeans.pdf)。
...
| 機能 | 実装と読み方 |
| --- | --- |
| テーマ | 9方式のクラスタリング＋NMF・LDA・BERTopic。代表語を表示し、未分類・情報不足は別集計 |
| 技術マップ | 簡易版は最大400論文、全件対応版は全対象を共通座標へ配置して重心を集計（描画だけ最大400点）。年・四半期・月別層、重心・内容変化・近傍論文の評論を表示。点は論文、類似線は文書表現に基づき、赤矢印は期間別重心の差。引用関係の線ではありません |
