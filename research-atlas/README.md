...
上限は20MB・10,000論文です。無効な行や重複を報告し、空欄の引用数は未知のまま保持します。検索結果の一部のみを出力すると、その部分集合の分析になります。
...
- データセットと結果は `data/` にJSONで保存します。`.env`、`data/`、仮想環境はGit対象外です。
...
node --test tests/test_csv_ui.mjs tests/test_foresight_ui.mjs tests/test_restore_ui.mjs tests/test_connection_settings_ui.mjs
```
