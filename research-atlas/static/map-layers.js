...
(function(global){
  ...
  function arrowGeometry(movement,options={}){
    if(movement.status==='insufficient'||!movement.from||!movement.to)return null;
    const distance=Math.hypot(movement.to.x-movement.from.x,movement.to.y-movement.from.y);
    if(!Number.isFinite(distance)||distance<.002)return null;
  ...
  function render(context,data,prefs){
    ...
    const arrows=selectedMovements.map(m=>{const options=layerOpt(m.from_period),g=arrowGeometry(m,options);if(!g)return '';return `<path class="centroid-arrow ${m.status==='stable'?'centroid-arrow-stable':''} ${prefs.movement===m.id?'selected':''}" data-motion-key="arrow-${e(m.id)}" d="${g.path}" marker-end="url(#centroid-red-arrow)" data-landscape-movement="${e(m.id)}" role="button" tabindex="0" aria-label="${e(m.from_period)}から${e(m.to_period)}の重心移動。${m.status==='shift'?'内容変化の候補':'内容変化は未確認'}"><title>${e(m.from_period)} → ${e(m.to_period)} / 同一平面の移動距離 ${fmt(g.distance)}</title></path>`;}).join('');
