...
(() => {
  'use strict';
  const cache=new Map(), landscapeCache=new Map(), preferences={mode:'relief',contours:true,height:65,projection:'auto',interval:'year',topic:null,start:null,movement:null,provider:'none',scope:'sample',centroid:null,camera:{yaw:-.16,pitch:.72}};
  let latest=null;
  ...
  if(window.__ATLAS_UI_TEST__)window.__landscapeTest={cache,landscapeCache,preferences,fetchTerrain,fetchLandscape,controls,scene,refresh,advancedControls,selectedMovement,selectedCentroid,selectTopic,selectCentroid,visibleMovements,generateReport,centroidPanel,testScopeNote,get report(){return report;}};
})();
