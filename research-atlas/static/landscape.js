...
(() => {
  ...
  function advancedView(context){
    ...
    return `<div class="landscape-root" data-landscape-result="${context.e(context.result.id)}" data-landscape-key="${context.e(landscapeKey(context))}">${advancedControls(context,entry)}${data?window.AtlasLayers.render(context,data,preferences):scene(context,null)}<div class="landscape-reading-key"><span><i>⊙</i> 点滅 = 話題の重心</span><span><b>→</b> 赤 = 同一平面上の重心移動</span><span>破線 = 内容変化は未確認</span></div><p class="terrain-explanation">全期間に一度だけ投影した共通座標を使用します。時層の高さは時間、立体の高さ・等高線は表示論文の相対密度です。矢印は前の期間の平面上に投影し、層の高さの差を移動量に含めません。${data?`表示対象 ${context.num(data.map.nodes.length)}論文による探索分析です。全論文の統計とは区別してください。`:''}</p>${data?(data.warnings||[]).map(w=>`<p class="terrain-explanation">${context.e(w)}</p>`).join('')+driftPanel(context,data):''}</div>`;
  }
