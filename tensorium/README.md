...
さらに **学習履歴・比較**（全実行の一覧・名前編集・削除・代表指標の比較チャート）と
...
| POST | `/api/train` | `{spec, family, model, hparams, name}` → `job_id`（同時実行は 1 本） |
...
| GET/POST | `/api/settings` | デバイス・プロキシ・HF ミラー・オフライン等 |
...
tensorium/
  ...
  tcore/
    ...
    engine/
      datasetup.py     ファミリー共通の準備・評価（torch 非依存）
      baseline.py      ベースライン
