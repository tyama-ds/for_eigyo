...
### 生成AI(LLM)のプロキシと接続診断
- LLM の接続は**接続先URLごと**にプロキシを解決します。`NO_PROXY` に合致するホスト
  (`localhost` / `127.0.0.1` / `host.docker.internal` 等)は、プロキシ設定があっても
  **必ず直接接続**します — 社内プロキシ環境でローカルLLM(Ollama 等)に接続できない
  典型的な事故を防ぎます。環境変数の一般プロキシは設定ファイルに保存しません
  (保存されるのは GUI で明示したプロキシと `LLM_PROXY` のみ)。
- 接続テスト(GUI「接続テスト」/ `POST /api/settings/llm/test`)は失敗理由
  (HTTPステータス・サーバのエラーメッセージ・接続エラー種別)と接続経路
  (接続先・プロキシ適用状況)を表示します。APIキー・プロキシ資格情報は表示しません。
- `response_format`(JSONモード)非対応のローカルLLMサーバが 400 を返す場合は、
  自動的に外して再試行します。
