...
## D-016: 監査(第2次)対応 — 期間/単位・列意味抽出・出典権威・状態原子性・配布

- **期間を単位に含める**: フロー質問の目標単位を期間つき(item/day, JPY/month 等)で
  全工程(生成・式・証拠・検算・表示)に一貫させる。フロー判定はレート/継続表現に限定し
  「2021年」「1月1日」の部分一致で誤判定しない。検算は目標単位が『直接比較可能(次元一致
  かつ倍率≈1)』でない限り数値比較せず「検算不成立(incompatible)」とする。
- **列意味ベース抽出**: 表・CSVは一致行の最初の数値ではなく、ヘッダーの意味(年・地域列は
  値に採らない/別名・期待単位・値語で値列を決定)で抽出する。単位はセル→列ヘッダーの順で
  推定し、根拠が無ければ空欄(目標単位を無条件採用しない)。
- **出典の権威性**: 政府・国際機関を自称する高権威(S/A)は、信頼ドメインまたは一次資料URLで
  裏付けられない限り昇格させない。裏付けなしは要確認・減点。定義/母集団の比較は正規化して
  表記揺れだけでは非互換にしない。地域・期間の互換性は値の乖離率に関係なく統合前に判定し、
  非互換な証拠は平均・中央値・分位点から除外する(全国/全世界値は按分の仮定つきで許容)。
- **状態の原子性と無効化**: すべての編集APIはキャッシュの deep copy 上で検証・再計算し、
  全成功時のみ commit(失敗時はキャッシュ/DBを変更前に保つ)。スコープ変更(+再生成)は
  モデル・パラメータを完全置換し、旧スコープの証拠・批判・矛盾・シミュレーション・検算・
  信頼度を破棄する。同値保存は何も変えない。主=検算の同一ID・単位検査不合格の主モデルは不可。
- **APIキー分離**: 環境変数解決は設定ロード時のみ。プロバイダは api_key=None(未指定→env)と
  ""(明示的にキー無し→env補完しない)を区別。接続先変更で新キーが無ければ旧キーを流用しない。
  clear_api_key で明示削除。Host ヘッダ許可リスト(DNSリバインディング対策)を追加。
- **配布**: 設定YAML・静的ファイルを wheel 同梱(fermiscope/_bundled)し、importlib 相当の
  リソース解決で非editable install でも起動。書き込みデータ(DB・LLM設定)はデータ
  ディレクトリへ。必須リソース不足は起動時に明示エラー(黙って空へフォールバックしない)。
