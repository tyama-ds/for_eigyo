...
function extractChartDataset(request, ctx) {
  // JSON整形の結果、または繰り返し実行（foreach）の数値結果をデータソースにできる
  const dataPrev = [...ctx.results].reverse().find((r) => r.ok && !r.superseded && (
    (r.toolId === "json-tool" && r.data !== undefined)
    || (r.step?.foreach && Array.isArray(r.data))
  ));
  if (dataPrev) {
    const ds = jsonToDataset(dataPrev.data);
    if (ds) {
      return { ...ds, source: dataPrev.toolId === "json-tool" ? "JSON整形の結果" : "繰り返し実行の結果" };
    }
  }
...
const BUILTIN_TOOLS = [
  ...
  },
  {
    ...
    run(input, ctx) {
      // foreach やパイプで個別入力が渡された場合はそちらを優先する
      const target = (input && input !== ctx.request)
        ? (extractQuoted(input) || input)
...
function planWithRules(request) {
  ...
  // 承認ゲート: 「確認しながら」等で全ステップに、メモ全削除は常に承認を要求
  const approveAll = /確認しながら|承認し(ながら|てから)|(一つ|1つ)ずつ確認/.test(request);
  for (const s of steps) {
    if (approveAll) s.approval = true;
    if (s.toolId === "memory" && /(忘れて|全部?削除|メモをクリア)/.test(request)) s.approval = true;
  }
  return steps;
}
...
/* ---- 繰り返し（foreach）: 1ステップを項目ごとに反復実行する ---- */

const FOREACH_MAX = 8;   // 1ステップの繰り返し上限

/** step.foreach（配列 / 「前の結果の各行・各項目」/ 区切りつき文字列）を項目リストへ解決 */
function resolveForeachItems(spec, ctx) {
  if (Array.isArray(spec)) {
    return spec.map((s) => String(s).trim()).filter(Boolean).slice(0, FOREACH_MAX);
  }
  const s = String(spec || "").trim();
  if (!s) return [];
  if (/前の結果|直前の結果/.test(s)) {
    const lastOk = [...ctx.results].reverse().find((r) => r.ok && !r.superseded);
    if (!lastOk) return [];
    if (Array.isArray(lastOk.data)) {
      return lastOk.data.map((d) => (typeof d === "object"
        ? (d.label !== undefined ? String(d.label) : JSON.stringify(d)) : String(d)))
        .filter(Boolean).slice(0, FOREACH_MAX);
    }
    const text = (lastOk.text || "").replace(/[*`#>]/g, "");
    const parts = /各行/.test(s) ? text.split(/\n/) : text.split(/[\n、,;・]/);
    return parts.map((p) => p.replace(/^[-・\s]+/, "").trim())
      .filter((p) => p.length >= 1).slice(0, FOREACH_MAX);
  }
  return s.split(/[、,;\/\n]/).map((p) => p.trim()).filter(Boolean).slice(0, FOREACH_MAX);
}

/** 項目を入力テンプレートに埋め込む（{{項目}} が無ければ項目そのものを入力にする） */
function foreachItemInput(template, item) {
  if (/\{\{\s*(項目|item)\s*\}\}/i.test(template)) {
    return template.replace(/\{\{\s*(項目|item)\s*\}\}/gi, item);
  }
  return item;
}

/* ---- 承認ゲート: 実行前にユーザーの許可を取る（入力の編集も可能） ---- */

/** 実行カード内に承認ボックスを出し、ユーザーの判断を待つ。
    resolve は {ok: true, input: 編集後の入力} または {ok: false}。 */
function requestApproval(card, step, agent, tool, input) {
  return new Promise((resolve) => {
    const box = document.createElement("div");
    box.className = "approval-box";
    box.innerHTML = `
      <div class="approval-head">⏸ 承認待ち — ${agent.icon} ${escapeHtml(agent.name)} が
        「${escapeHtml(step.title)}」（${tool.icon} ${escapeHtml(tool.name)}）を実行しようとしています</div>
      <label class="approval-label">ツールへの入力（編集できます）</label>
      <textarea class="approval-input" rows="2"></textarea>
      <div class="approval-actions">
        <button type="button" class="ghost-btn" data-skip>スキップ</button>
        <button type="button" class="primary-btn" data-ok>実行を許可</button>
      </div>`;
    $(".approval-input", box).value = input;
    const done = (result) => { box.remove(); resolve(result); };
    $("[data-ok]", box).addEventListener("click", () =>
      done({ ok: true, input: $(".approval-input", box).value }));
    $("[data-skip]", box).addEventListener("click", () => done({ ok: false }));
    const toggle = $(".run-toggle-log", card);
    if (toggle) card.insertBefore(box, toggle);
...
同じツールを複数の項目に繰り返すステップには "foreach" を付けられます:
- "foreach": ["項目1", "項目2", ...]（最大8件） または "前の結果の各項目"
- その場合 input は「{{項目}}」を含むテンプレートにする

破壊的な操作や判断が分かれるステップには "approve": true を付けると、
実行前にユーザーの承認を求めます。

依頼: ${request}

次の形式の JSON 配列のみを出力:
[{"agent": "エージェントid", "tool": "ツールid", "input": "そのツールに渡す入力", "title": "ステップ名(8文字以内)", "when": "条件(任意)", "foreach": [...], "approve": false}]`,
    max_tokens: 1200,
  });
  ...
  try {
    ...
    for (const s of arr.slice(0, 8)) {
      ...
      const foreach = Array.isArray(s.foreach)
        ? s.foreach.map(String).slice(0, FOREACH_MAX)
        : (typeof s.foreach === "string" && s.foreach.trim() ? truncate(s.foreach, 80) : undefined);
      steps.push({
        toolId: tool.id,
        ...
        foreach: (Array.isArray(foreach) && !foreach.length) ? undefined : foreach,
        approval: s.approve === true || s.approval === true || undefined,
      });
    }
...
async function runToolStep(card, chipId, step, ctx, emitLog, run) {
  ...
  let input = resolvePipes(step.input, ctx);
  // 承認ゲート: ユーザーの許可を待つ（入力の編集も受け付ける）
  if (step.approval) {
    setStepState(card, chipId, "running");
    emitLog("orchestrator", `⏸ 「${step.title}」はユーザーの承認待ちです`);
    const ap = await requestApproval(card, step, agent, tool, input);
    if (!ap.ok) {
      emitLog("orchestrator", `ユーザーの判断で「${step.title}」をスキップしました`);
      setStepState(card, chipId, "skipped");
    ...
    if (ap.input !== input) {
      input = ap.input;
      emitLog("orchestrator", `ユーザーが実行を許可し、入力を編集しました: 「${truncate(input, 60)}」`);
    } else {
      emitLog("orchestrator", "ユーザーが実行を許可しました");
    }
  }
  setStepState(card, chipId, "running");
  ...
  try {
    if (input !== step.input && !step.approval) {
      emitLog("orchestrator", `「${step.title}」の入力に前ステップの結果を注入しました`);
    }
    let result;
    if (step.foreach) {
      // 繰り返し実行: 項目リストに展開し、1件ずつ同じツールを回して集約する
      const items = resolveForeachItems(step.foreach, ctx);
      if (!items.length) throw new Error("繰り返しの対象項目が見つかりませんでした");
      emitLog(agent.id, `🔁 ${tool.name} を ${items.length} 件に繰り返し実行します`);
      const parts = [];
      for (let k = 0; k < items.length; k++) {
        const itemInput = foreachItemInput(input, items[k]);
        emitLog(agent.id, `（${k + 1}/${items.length}）入力: 「${truncate(itemInput, 50)}」`);
        try {
          const r = await tool.run(itemInput, ctx);
          parts.push({ item: items[k], ok: true, text: r.text, data: r.data, html: r.html });
        } catch (e) {
          parts.push({ item: items[k], ok: false, text: String(e.message || e) });
        }
        await sleep(160);
      }
      const okN = parts.filter((p) => p.ok).length;
      if (!okN) throw new Error(`全 ${items.length} 件が失敗しました（例: ${parts[0].text}）`);
      const lines = parts.map((p) => {
        const body = p.ok
          ? truncate(p.text.replace(/\*\*/g, "").replace(/[`#>]/g, "").replace(/\s*\n\s*/g, " / ").trim(), 140)
          : "⚠️ " + p.text;
        const shownItem = truncate(p.item, 30).replace(/\*/g, "×").replace(/`/g, "'");
        return `- **${shownItem}** → ${body}`;
      });
      // 全件が数値の結果なら {label, value} の配列にして、後続のグラフ/表がそのまま使えるようにする
      const okParts = parts.filter((p) => p.ok);
      const numeric = okParts.length && okParts.every((p) => typeof p.data === "number");
      result = {
        text: `🔁 ${items.length} 件を繰り返し実行（成功 ${okN}/${items.length}）:\n${lines.join("\n")}`,
        html: parts.filter((p) => p.html).map((p) => p.html).join(""),
        data: numeric
          ? okParts.map((p) => ({ label: truncate(p.item, 14), value: p.data }))
          : parts.map((p) => (p.data !== undefined ? p.data : p.text)),
      };
    } else {
...
function fallbackAnswer(request, ctx) {
  ...
  const parts = [];
  if (!ok.length && !ng.length && ctx.skipped) {
    return `⏸ 承認されなかったため、${ctx.skipped} 件のステップをスキップしました。実行された処理はありません。`;
  }
...
const SAMPLES = [
  ...
  "覚えて: 定例会議は毎週金曜 15:00",
  "メモを一覧して",
  "「りんごジュース、みかん、バナナケーキ」のそれぞれの文字数を数えて",
  "確認しながら: 100 × 25 を計算して、結果をメモに保存して",
  "サイコロを3個振って",
